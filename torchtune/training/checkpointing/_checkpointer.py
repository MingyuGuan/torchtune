...
class FullModelTorchTuneCheckpointer(_CheckpointerInterface):
    """
    Create a hash directory to store hash_checkpoint in the same manner as recipe_chekpoint
    Checkpointer which reads and writes checkpoints in a format compatible with
    torchtune. No conversion of weights is required.
    ...
    def __init__(
        ...
        adapter_checkpoint: Optional[str] = None,
        recipe_checkpoint: Optional[str] = None,
        hash_chekpoint: Optional[str] = None,
        resume_from_checkpoint: bool = False,
    ) -> None:
        # Fail fast if ``checkpoint_files`` is invalid
        if len(checkpoint_files) != 1:
...
class FullModelHFCheckpointerHash(_CheckpointerInterface):
    """
